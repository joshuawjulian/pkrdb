import { getNextRoundOption } from './action';
import { actionComplete, getCurrentRound, getLargestWager, getLargestWagerAmount, getMinMaxBet, getSeatOrder, getStacksAtStartOfCurrentRound, getWagers, hasNonBlindAction, hasNonBlindStraddleAction, playerActionsCurrentRound, } from './engineUtils';
...
export let next = (state) => {
    ...
    if (actionComplete(state))
        return [getNextRoundOption(currentRound)];
    ...
    let largestRemainingStackBehind = Math.max(...stacksBehind.filter((stack) => stack !== 'unknown'));
    ...
    if (wagers.length > 0) {
        ...
        if (largestWagerAmount < remainingStackBehind) {
            ...
            if (!hasPlayerActed) {
                ...
                let seatActionIdx = playerActions.findLastIndex((action) => 'seat' in action && action.seat === currSeat);
                // if the players last action was call/bet there will
                // be an amount, otherwise its check and 0
                let smallBet = 'amount' in playerActions[seatActionIdx]
