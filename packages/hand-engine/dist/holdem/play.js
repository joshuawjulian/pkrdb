...
export let sixHandedSimple = GameStateSchema.parse({
    players: [
        {
            startingStack: 100,
            cards: [
...
export let cardsToString = (cards) => {
    return `${cards.rank}${cards.suit}`;
};
export let displaySeats = (state) => {
    let playerTable = [];
...
export let displayOptions = (options) => {
...
export let displayActionList = (state) => {
    let actionTable = [];
...
export let getInput = async (prompt, schema) => {
...
while (true) {
    ...
    let option = await getInput('Choose an option: ', z.coerce
        .number()
        .min(0)
    ...
    if (act.action === 'fold' || act.action === 'check') {
        ...
        let betSize = await getInput(`Bet Amount (${act.min}-${act.max}) `, betSchema);
        state.actionList.push({
            action: act.action,
            seat: act.seat,
            amount: betSize,
